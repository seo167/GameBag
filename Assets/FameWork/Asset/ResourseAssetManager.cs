...
namespace FrameWork {
	...
	public class ResourseAssetManager:MonoBehaviour{
		...
		IEnumerator _LoadAssetBundle(string AssetBundlePath,UnityAction _ActionCall=null){
			...
			[SerializeField]List<string> _AssetBundleList=new List<string>();
		//正式获取资源 
			...
			if (_www.isDone && _www.error == null) {
				...
				for (int i = 0; i < manifestLoader.GetAllAssetNames().Length; ++i) {
					...
					#endif
			_AssetBundleList.Add (manifestLoader.GetAllAssetNames()[i]);
		...
		public void Clear(){
			Debug.Log ("清除数据");
			for(int i=0;i<_AssetBundleList.Count;++i){
				string str = _AssetBundleList [i];
				AssetDictionary.Remove (str);
				_AssetBundleList.Remove (str);
			}
			...
			void OnDestroy(){
				if(_ResourseAssetManager!=null){
				Debug.Log ("清除");
			GameObject.Destroy (_ResourseAssetManager.gameObject);
